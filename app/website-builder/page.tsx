...
type Project = { id?: string; name: string; prompt?: string; direction?: string; page?: { title?: string; blocks?: string[]; generated?: boolean; contentHtml?: string; previewUrl?: string; sourceUrl?: string; wordpressId?: number } };
...
export default function WebsiteBuilderPage() {
  ...
  function loadProject(project: Project) {
    ...
    setImportedPreviewUrl(project.page?.previewUrl || '');
  ...
  const draftPreview = importedPreviewUrl ? <div className="min-h-[680px] bg-white"><div className="flex items-center justify-between border-b border-os-border bg-os-surface px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-os-accent"><span>Imported site · local working copy</span><span>{blocks.length} pages loaded</span></div><iframe title="Imported website preview" src={importedPreviewUrl} className="h-[780px] w-full border-0 bg-white" /></div> : <div className="min-h-[680px] bg-[#f4f0e7] px-[8%] py-14 text-[#1c211d]"><div className="mx-auto max-w-3xl"><div className="flex items-center justify-between text-[11px] font-bold tracking-[0.14em]"><span>BUSINESS OS / WORKING DRAFT</span></div><div className="mt-20 max-w-2xl"><div className="mb-5 font-mono text-[10px] uppercase tracking-[0.2em] text-[#718074]">{selectedTemplate ? `Template · ${selectedTemplate}` : 'AI website draft'}</div><input aria-label="Draft title" value={title} onChange={(event) => setTitle(event.target.value)} className="w-full bg-transparent font-serif text-[clamp(42px,6vw,78px)] leading-[0.94] tracking-[-0.05em] outline-none" /><p className="mt-8 max-w-lg text-[17px] leading-relaxed text-[#566159]">{generated ? 'Your complete working draft is ready. Refine the structure, content, and conversion path before export.' : 'Your website preview will appear here. Start with a command or describe what you want to build.'}</p><button type="button" onClick={() => void generatePage()} className="mt-9 rounded-full bg-[#1c211d] px-6 py-3 text-[12px] font-bold text-[#f4f0e7]">{generated ? 'Regenerate draft' : 'Generate draft'} ↗</button></div></div></div>;
