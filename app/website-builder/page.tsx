...
import { WebsiteRevisionPanel } from '@/components/WebsiteRevisionPanel';
...
type Project = { id?: string; name: string; prompt?: string; direction?: string; page?: { title?: string; blocks?: string[]; generated?: boolean; contentHtml?: string; previewUrl?: string; pages?: Array<{ title: string; slug: string }>; sourceUrl?: string; wordpressId?: number } };
...
export default function WebsiteBuilderPage() {
  ...
  const [importedPreviewUrl, setImportedPreviewUrl] = useState('');
  const [importedPages, setImportedPages] = useState<Array<{ title: string; slug: string }>>([]);
  useEffect(() => { void fetch('/api/website/lifecycle').then(r => r.json()).then(async body => {
    if (!body.state?.projectId) return;
    const projects = await fetch('/api/website/projects').then(r => r.json());
    const canonical = projects.projects?.find((item: Project) => item.id === body.state.projectId);
    if (canonical) loadProject(canonical);
  }).catch(() => setAiStatus('Could not restore the canonical project. Open Saved Projects.')); }, []);
  ...
  function loadProject(project: Project) {
    ...
    setImportedPreviewUrl(project.page?.previewUrl ? `${project.page.previewUrl.replace(/\/$/, '')}/` : '');
    setImportedPages(project.page?.pages || []);
  ...
  async function generatePage(request = prompt) {
    ...
    if (projectId && importedPages.length) { const result = await requestPageWork(cleanRequest); setAiStatus(result); return; }
  ...
  async function handleCopilotCommand(message: string): Promise<BrainCommandResult | null> {
    if (projectId && importedPages.length) return { handled: true, message: await requestPageWork(message) };
  ...
  async function requestPageWork(request: string) {
    try {
      const { state } = await fetch('/api/website/lifecycle', { cache: 'no-store' }).then(r => r.json());
      if (!state || state.projectId !== projectId) return 'Select the canonical project and pilot page in Working Preview first.';
      const response = await fetch('/api/website/lifecycle', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'run', projectId, version: state.version, revisionId: state.selectedId, request }) });
      const body = await response.json();
      return response.ok ? 'Your request is queued for the selected page. G-Brain and specialist results will appear in Working Preview.' : body.error || 'Page work failed to queue.';
    } catch { return 'Could not reach the page worker. Please retry.'; }
  }

  const builderChatContext = `Website Builder shared project context. Active project ID: ${projectId || 'new draft'}. Project: ${title}. Imported project: ${importedContent ? 'yes' : 'no'}. Imported homepage content is the working source; preserve original files and create draft edits only. Active sections/pages: ${blocks.join(', ')}. Template: ${selectedTemplate || 'not selected'}. Direction: ${direction}. Route this same project context through G-Brain and Conductor to the website, content, SEO, accessibility, affiliate, brand, and social agents. Return reviewable drafts and never publish, delete, or change external systems without explicit approval.`;

  const fallbackDraftPreview = importedPreviewUrl ? <div className="min-h-[680px] bg-white"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-os-border bg-os-surface px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-os-accent"><span>Imported site · original HTML working copy</span><select aria-label="Imported HTML page" value={importedPreviewUrl} onChange={(event) => setImportedPreviewUrl(event.target.value)} className="border border-os-border bg-os-surface2 px-2 py-1 text-[10px] text-os-text">{importedPages.map(page => <option key={page.slug} value={`${projectId ? `/api/website/projects/${projectId}/preview/${page.slug === 'home' ? '' : `${page.slug}/`}` : importedPreviewUrl}`}>{page.title}</option>)}</select></div><iframe title="Imported website preview" src={importedPreviewUrl} className="h-[780px] w-full border-0 bg-white" /></div> : <div className="min-h-[680px] bg-[#f4f0e7] px-[8%] py-14 text-[#1c211d]"><div className="mx-auto max-w-3xl"><div className="flex items-center justify-between text-[11px] font-bold tracking-[0.14em]"><span>BUSINESS OS / WORKING DRAFT</span></div><div className="mt-20 max-w-2xl"><div className="mb-5 font-mono text-[10px] uppercase tracking-[0.2em] text-[#718074]">{selectedTemplate ? `Template · ${selectedTemplate}` : 'AI website draft'}</div><input aria-label="Draft title" value={title} onChange={(event) => setTitle(event.target.value)} className="w-full bg-transparent font-serif text-[clamp(42px,6vw,78px)] leading-[0.94] tracking-[-0.05em] outline-none" /><p className="mt-8 max-w-lg text-[17px] leading-relaxed text-[#566159]">{generated ? 'Your complete working draft is ready. Refine the structure, content, and conversion path before export.' : 'Your website preview will appear here. Start with a command or describe what you want to build.'}</p><button type="button" onClick={() => void generatePage()} className="mt-9 rounded-full bg-[#1c211d] px-6 py-3 text-[12px] font-bold text-[#f4f0e7]">{generated ? 'Regenerate draft' : 'Generate draft'} ↗</button></div></div></div>;
  const draftPreview = projectId && importedPages.length ? <WebsiteRevisionPanel projectId={projectId} /> : fallbackDraftPreview;
