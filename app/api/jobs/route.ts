...
export async function POST(request: Request) {
  ...
  if (body.action === 'run') {
    ...
    for (const job of db.localJobs.all().filter((item: any) => item.status === 'queued' || (item.status === 'retry' && item.attempts < 3)).slice(0, limit)) {
      if (job.type === 'website-page-revision') { await runWebsitePage(db, job.id); results.push({ id: job.id, status: db.localJobs.all().find(item => item.id === job.id)?.status || 'failed' }); continue; }
      if (job.type === 'website-project-review') { db.localJobs.update(job.id, 'failed', 'Legacy project-wide job. Select the canonical pilot page and run its specialists in Website Builder.'); results.push({ id: job.id, status: 'failed' }); continue; }
