...
export function renderOpenPageHtml(input: OpenPageDocument): string {
  ...
  const renderBlock = (item: OpenPageBlock): string => {
    const p = item.props;
    if (item.type === 'navbar') return `<nav class="op-nav"><strong>${typeof p.logoUrl === 'string' && p.logoUrl ? `<img src="${escapeHtml(p.logoUrl)}" alt="${text(p.brand, doc.name)}">` : ''}${text(p.brand, doc.name)}</strong><div>${list(p.links).map((link) => `<a href="#">${escapeHtml(link)}</a>`).join('')}</div></nav>`;
    ...
    if (item.type === 'image') return `<figure class="op-image">${typeof p.src === 'string' && p.src ? `<img src="${escapeHtml(p.src)}" alt="${text(p.alt, item.label)}">` : ''}${text(p.caption) ? `<figcaption>${text(p.caption)}</figcaption>` : ''}</figure>`;
