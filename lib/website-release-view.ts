...
export async function serveRelease(mode: ReleaseMode, parts: string[] = []) {
  try {
    ...
    const id = mode === 'preview' ? state?.selectedId : mode === 'staging' ? state?.stagedId : state?.publishedId;
    const revision = state?.revisions.find(item => item.id === id);
    if (!state?.sourceRoot || !revision) throw new Error(`${mode} revision is unavailable.`);
    if (mode === 'staging' && revision.stagedHash !== revision.hash) throw new Error('The staged revision failed its integrity check.');
    if (mode === 'site' && !state.releases.some(item => item.revisionId === id)) throw new Error('No published release is available.');
    const prefix = `/${mode}`;
    ...
    return new NextResponse(typeof data === 'string' ? data : new Uint8Array(data), { headers: { 'content-type': MIME[ext], 'cache-control': 'no-store', 'x-content-type-options': 'nosniff', 'content-security-policy': "sandbox allow-scripts; default-src 'self' https: data: blob:; style-src 'self' https: 'unsafe-inline'; script-src 'self' https: 'unsafe-inline'; connect-src 'none'; form-action 'none'; frame-src 'none'", 'x-website-revision': revision.id, 'x-website-release-mode': mode } });
  } catch (error) {
